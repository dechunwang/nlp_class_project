...
class bert_loader:
    def __init__(self, directory, prefix_length, suffix_length):
        ...
        if os.path.isfile('storyset_raw.cache'):
            with open('storyset_raw.cache', 'rb') as f:
            ...
            for rows in story_set.iterrows():
                ...
                prefix = [word for word in self.tokenizer.tokenize(" ".join(list(rows[1][2:6]))) if word not in string.punctuation]
                ...
                suffix_bad = [word for word in self.tokenizer.tokenize(random.choice(story_end)) if word not in string.punctuation]
        ...
        with open('storyset_raw.cache', 'wb') as g:
...
if __name__ == '__main__':
    data = bert_loader('../dataset/',  100, 20)
    text, label = data[0]
